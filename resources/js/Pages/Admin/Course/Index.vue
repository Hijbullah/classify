<template>
    <admin-layout>
        <div>
            <h2 class="text-xl text-gray-900 font-semibold border-b-2 border-gray-900">All Courses</h2>
                ...
                <div class="bg-white rounded shadow overflow-x-auto">
                    <table class="w-full whitespace-no-wrap">
                        <tr class="text-left">
                            ...
                            <th class="px-6 pt-4 pb-4 font-normal text-center">Batch (s)</th>
                            <th class="px-6 pt-4 pb-4 font-normal text-center">Admission Status</th>
                        ...
                        <tr v-for="course in courses.data" :key="course.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
                            ...
                            <td class="border-t px-6 py-2 text-center">
                                <span v-if="course.admission_status" class="inline-flex items-center px-3 py-1 bg-green-500 text-white text-xs font-bold rounded-full uppercase tracking-wider">open</span>
                                <span v-else class="inline-flex items-center px-3 py-1 bg-red-500 text-white text-xs font-bold rounded-full uppercase tracking-wider">closed</span>
                            </td>
                            ...
                            <td class="border-t px-6 py-2 text-center space-x-2">
                                ...
                                <button 
                                    ...
                                    @click.prevent="deleteCourse(route('admin.courses.destroy', course.slug))"
                                >
        ...
        watch: {
            form: {
                handler: _.throttle(function() {
                    ...
                    this.$inertia.replace(route('admin.courses.index', Object.keys(query).length ? query : { remember: 'forget' }))
                }, 150),
                deep: true,
            },
        },
